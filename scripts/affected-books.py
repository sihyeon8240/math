...
import yaml
from book_manifest import SLUG_PATTERN, load_manifest
...
def manifest_at(revision: str | None) -> dict | None:
    ...
    try:
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("books"), list):
            return None
        slugs: set[str] = set()
        for record in raw["books"]:
            ...
            if slug in slugs:
                return None
            slugs.add(slug)
        # Historical books may have been deleted. Create only validated paths
        # to let the shared loader check their original metadata and policy.
...
def dependencies(slug: str) -> set[str]:
    ...
    while pending:
        ...
        if source in seen or not source.is_file():
            continue
        seen.add(source)
        text = without_comments(source.read_text(encoding="utf-8", errors="ignore"))
    ...
    if paths is None:
        ...
        for slug, inputs in dependency_map.items():
            if path_set & inputs or any(
                path.startswith(f"books/{slug}/") for path in path_set
            ):
                selected_set.add(slug)
        if rebuild_all or path_set & (BUILD_INFRA | IMAGE_INPUTS):
        ...
        if any(path not in known and not path.startswith(ignored) for path in path_set):
            selected_set.update(slugs)
