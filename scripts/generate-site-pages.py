...
def generate(
    *,
    ...
    if book:
        ...
        manifest = {**manifest, "books": [registered[book]]}
    selected_pages = render_site_pages(manifest)
    expected_names = set(selected_pages)
    if check:
        actual_names = (
            {path.name for path in pages_dir.glob("*.md")}
            if pages_dir.is_dir()
            else set()
        )
        ...
        if stale or orphaned:
            ...
            for name in orphaned:
                print(f"error: orphan generated site page: {name}", file=sys.stderr)
            return 1
...
def main() -> int:
    ...
    try:
        return generate(check=args.check, pages_dir=args.output_dir, book=args.book)
