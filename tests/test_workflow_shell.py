...
""",
    ...
    def test_immutable_tag_check_covers_only_image_inputs(self):
        text = (ROOT / "scripts/check-image-tag.sh").read_text(encoding="utf-8")
        self.assertIn(".devcontainer/Dockerfile scripts/check-toolchain.sh", text)
        self.assertNotIn("scripts/check-environment.sh", text)
    ...
    def test_image_registry_login_only_runs_for_new_tags(self):
        workflow = yaml.safe_load(
            (ROOT / ".github/workflows/build-image.yml").read_text(encoding="utf-8")
        )
