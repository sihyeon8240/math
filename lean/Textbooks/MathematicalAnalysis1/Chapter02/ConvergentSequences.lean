...
/-- Lemma (Supporting lemma): Representation bridge to the standard Mathlib limit. -/
...
/-- Lemma (Core result): a finite initial segment and a bounded tail have a common bound. -/
...
/-- Lemma (Core result): two distinct limits contradict the triangle inequality. -/
...
/-- Proposition (Core result): boundedness and uniqueness, paired with the printed theorem. -/
...
/-- Lemma (Core result): finite exceptional indices, rather than finitely many values. -/
...
/-- Lemma (Core result): Accepted Archimedean arithmetic supplies an index with reciprocal below ε. -/
...
/-- Lemma (Core result): Core example: the shifted reciprocal sequence converges to zero. -/
...
/-- Lemma (Core result): Core example: any positive-subspace limit would also be a real limit,
...
/-- Lemma (Core result): choose a distinct point within radius 1/(n+1). -/
...
/-- Theorem (Core result): Both parts of the neighborhood characterization in the textbook. -/
