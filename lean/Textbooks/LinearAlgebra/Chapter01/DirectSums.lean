...
theorem sum_eq_sup (U W : Submodule K V) : sumSubspace U W = U ⊔ W := by
  ...
  · rintro x ⟨u, hu, w, hw, rfl⟩
    exact (U ⊔ W).add_mem ((show U ≤ U ⊔ W from le_sup_left) hu) ((show W ≤ U ⊔ W from le_sup_right) hw)
  · apply sup_le
    · intro u hu
...
theorem directSum_of_sup_inf (U W : Submodule K V) (hspan : U ⊔ W = ⊤)
  ...
  intro x
  have hx : x ∈ U ⊔ W := by rw [hspan]; trivial
...
theorem independent_disjoint_spans {s t : Set V} (hi : LinearIndepOn K id (s ∪ t))
  ...
  apply bot_unique
  intro x hx
  have hs : x ∈ span K (id '' s) := by simpa only [image_id, SetLike.mem_coe] using hx.1
  have ht : x ∈ span K (id '' t) := by simpa only [image_id, SetLike.mem_coe] using hx.2
...
theorem union_is_basis [DecidableEq V] {s t : Finset V} {U W : Submodule K V}
  ...
  · intro a ha x hx
    ...
    have htz : (∑ y ∈ t, a y • y) = 0 := by simpa only [hsz, zero_add] using ha
