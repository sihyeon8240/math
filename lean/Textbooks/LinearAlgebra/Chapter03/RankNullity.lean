...
theorem rank_nullity [Module.Finite K V] (f : V →ₗ[K] W) :
    Module.finrank K V = Module.finrank K (LinearMap.ker f) + Module.finrank K (LinearMap.range f) := by
...
theorem finrank_top [Module.Finite K V] : Module.finrank K (⊤ : Submodule K V) = Module.finrank K V := by
...
theorem eq_zero_of_finrank_zero [Module.Finite K V] (h : Module.finrank K V = 0) (x : V) : x = 0 := by
  obtain ⟨s, hs⟩ := hasFiniteBasis_iff_finite.mpr (inferInstance : Module.Finite K V)
  have hc : s.card = 0 := by
    exact (dimension_eq_card ⟨s, hs⟩ hs).symm.trans ((dimension_eq_finrank ⟨s, hs⟩).trans h)
  have he : s = ∅ := Finset.card_eq_zero.mp hc
  have hx : x ∈ Submodule.span K (s : Set V) := by rw [hs.2]; trivial
...
theorem bijective_of_surjective_of_finrank_eq [Module.Finite K V] [Module.Finite K W]
  ...
  have hk : Module.finrank K (LinearMap.ker f) = 0 := by omega
