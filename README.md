...
|---|---|---|
| Elementary Number Theory | `elementary-number-theory` | `make books BOOK=elementary-number-theory` |
| Linear Algebra | `linear-algebra` | `make books BOOK=linear-algebra` |
| Mathematical Analysis | `mathematical-analysis` | `make books BOOK=mathematical-analysis` |

<!-- END GENERATED BOOKS -->

`books.yml` is the machine-readable central registry. Each book README contains its scope notes and generated assembly, reference, build, and feedback details.
...
| Run the full validation suite | `make check all strict` |
...
`books.yml` is the canonical textbook registry. Build output belongs only under the ignored `build/` directory. See the [repository architecture](docs/ARCHITECTURE.md) for file ownership, assembly rules, metadata, and generated files.
