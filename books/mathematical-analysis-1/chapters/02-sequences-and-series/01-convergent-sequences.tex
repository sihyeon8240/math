...
\begin{definition} \label{an1:def:sequence-convergence}
  ...
  Eventually every term is within any prescribed positive distance of $p$.

  \begin{lean}
    def convergesTo (u : ℕ → X) (p : X) : Prop :=
      ∀ ε : ℝ, 0 < ε → ∃ N : ℕ, ∀ n ≥ N, dist (u n) p < ε
  \end{lean}
  A convergent sequence has a limit in its ambient space.

  \begin{lean}
    abbrev convergent (u : ℕ → X) : Prop := ∃ p : X, convergesTo u p
  \end{lean}
  A divergent sequence has no limit in that space.
...
\begin{lemma} \label{an1:lem:convergesTo-iff-tendsto}
  The epsilon definition of $u_n\to p$ agrees with the standard Lean limit of a sequence.
...
\begin{proof}
  The neighborhoods of a point in a metric space have positive-radius balls as a basis, and the eventual sets of natural indices have tails as a basis. Expressing the limit with these bases gives exactly the stated epsilon condition.
...
\begin{definition} \label{an1:def:bounded-sequence}
  ...
  Boundedness concerns the set of values of the sequence.
...
\begin{proof}
  ...
  For $n<N$, the distance $d(u_n,p)$ is at most the finite sum defining $R-1$. For $n\geq N$, it is less than $1\leq R$. Thus every term lies in $N_R(p)$. The argument still works if the initial segment is empty.
...
\begin{theorem} \label{an1:thm:neighborhood}
  A sequence converges to $p$ exactly when every neighborhood $N_\varepsilon(p)$ contains all but finitely many of its terms, counted by indices. Also, if $p\in E^{\prime}$, a sequence in $E\setminus\{p\}$ converges to $p$.
...
\begin{proof}
  The finite-exception characterization proves the first assertion, including its converse. The construction from shrinking deleted neighborhoods proves the second assertion, with the terms explicitly required to belong to $E\setminus\{p\}$.
