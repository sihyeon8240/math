...
\begin{definition} \label{an1:def:sequence-convergence}
  ...
  We write $u_n\to p$ or $\lim_{n\to\infty}u_n=p$ and represent this by \texttt{Filter.Tendsto u Filter.atTop (nhds p)}.
...
\begin{proof}
  Positive-radius balls form a neighborhood basis at the limit, and tails form a basis for eventual natural indices. These two bases give precisely the epsilon condition.
...
\begin{definition} \label{an1:def:bounded-sequence}
  A sequence is \textbf{bounded} if its range $\{u_n:n\in\N\}$ is bounded, represented by \texttt{Bornology.IsBounded (Set.range u)}.
\end{definition}
