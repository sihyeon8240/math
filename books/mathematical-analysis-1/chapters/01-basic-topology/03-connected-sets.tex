\begin{definition} \label{an1:def:connected-set}
  ...
  The empty set is connected under this convention. We therefore use Mathlib's \texttt{IsPreconnected E}; \texttt{IsConnected E} additionally requires $E$ to be nonempty.
  The following lemma relates the standard predicate to the separation criterion.
\end{definition}

% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.ConnectedSets.isPreconnected_iff
\begin{lemma} \label{an1:lem:isPreconnected-iff}
  $E$ is preconnected if and only if it has no decomposition into two nonempty separated sets.
...
\begin{proof}
  Use the closed-cover characterization of preconnectedness. If $E=A\cup B$ is a separation, the closed sets $\overline A$ and $\overline B$ cover $E$ and both meet it. Preconnectedness supplies a point of $E$ in both closures, contradicting separation whichever part contains it.
  ...
  Conversely, let closed sets $F,G$ cover $E$ and both meet it. If their intersection misses $E$, then $E=(E\cap F)\cup(E\cap G)$. The closure of $E\cap F$ lies in $F$, and the closure of $E\cap G$ lies in $G$, by the previously proved closure property. Thus the two nonempty parts are separated, a contradiction.
...
\begin{lemma} \label{an1:lem:closure-upper-bound}
  If every point of $A\subseteq\R$ is at most $c$, the same is true for every point of $\overline A$.
...
\begin{proof}
  If $x\in\overline A$ were greater than $c$, choose a point $y\in A$ within $x-c$ of $x$. Then $|x-y|<x-c$ would imply $y>c$, contradicting the upper bound.
...
\begin{lemma} \label{an1:lem:closure-lower-bound}
  If every point of $A\subseteq\R$ is at least $c$, the same is true for every point of $\overline A$.
...
\begin{proof}
  If $x\in\overline A$ were less than $c$, a point $y\in A$ within $c-x$ of $x$ would satisfy $y<c$. This contradicts the lower bound.
...
\begin{lemma} \label{an1:lem:no-separation-of-between}
  Suppose $E\subseteq\R$ contains every point strictly between any two of its elements.
...
\begin{proof}
  Let $S=A\cap[a,b]$. It contains $a$ and is bounded above by $b$, so $c=\sup S$ exists and satisfies $a\leq c\leq b$. The supremum is in $\overline S\subseteq\overline A$.
  ...
  Choose $\delta=\min(r,b-c)/2$. It is positive, less than both $r$ and $b-c$, and $x=c+\delta$ lies strictly between $a$ and $b$. Hence $x\in E$.
  ...
  The distance $d(c,x)=\delta<r$ excludes $x$ from $B$. Therefore $x\in A\cap[a,b]=S$, yet $x>c=\sup S$, a contradiction. Choosing half the available radius ensures that the new point is inside the excluded ball, not merely on its boundary.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.ConnectedSets.isPreconnected_iff_between
\begin{theorem} \label{an1:thm:connected-sets-in-R}
  A set $E\subseteq\R$ is connected if and only if, whenever $a,b\in E$ and $a<x<b$, one has $x\in E$.

  \begin{lean}
    theorem isPreconnected_iff_between (E : Set ℝ) :
        IsPreconnected E ↔ ∀ a ∈ E, ∀ b ∈ E, ∀ x : ℝ, a < x → x < b → x ∈ E
  \end{lean}
\end{theorem}

\begin{proof}
  Suppose $E$ is connected and a point $x$ between $a,b\in E$ were missing. Split $E$ into $A=E\cap(-\infty,x)$ and $B=E\cap(x,\infty)$. They cover $E$, and $a\in A$, $b\in B$ make both nonempty.
  ...
  The closure of $A$ stays at or below $x$, so it misses $B$. Similarly the closure of $B$ stays at or above $x$, so it misses $A$. This is a forbidden separation.
  ...
  Conversely, suppose every intermediate point is in $E$ and assume a separation $E=A\cup B$. Choose $a\in A$ and $b\in B$; separation makes them distinct. If $a<b$, the preceding supremum argument contradicts the separation. If $b<a$, interchange the two sets and apply the same argument.
