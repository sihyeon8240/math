\begin{definition} \label{an1:def:metric-space}
  ...
  In Lean, this is the standard assumption \texttt{[MetricSpace X]}.
  For every $n\in\N$, Euclidean space $\R^n$ is represented by \texttt{EuclideanSpace} over $\R$ indexed by \texttt{Fin n}, including dimension zero.
  We use ordinary set and function language throughout: a space is represented by a type, a subset by \texttt{Set X}, and a subset regarded as a space by its subtype. This interpretation does not identify Lean's type-theoretic foundations with the ZFC axiom system.
\end{definition}
...
\begin{definition} \label{an1:def:neighborhood-open-ball}
  ...
  \]
  It is represented by \texttt{Metric.ball p r}; metric symmetry allows the center in either distance argument.
  The \textbf{punctured ball} is $B(p,r)\setminus\{p\}$.
  ...
  The notation $U\in\mathcal N(p)$, represented by membership in \texttt{nhds p}, means that $U$ is a neighborhood of $p$.
\end{definition}

% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.mem_nhds_iff
\begin{lemma} \label{an1:lem:mem-nhds-iff}
  $U$ is a neighborhood of $p$ if and only if it contains $B(p,r)$ for some $r>0$.

  \begin{lean}
    theorem mem_nhds_iff (E : Set X) (p : X) :
        E ∈ nhds p ↔ ∃ r > 0, ball p r ⊆ E
  \end{lean}
\end{lemma}

\begin{proof}
  The metric neighborhood interface expresses exactly the contained-ball condition.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.mem_punctured_ball_iff
\begin{lemma} \label{an1:lem:deleted-neighborhood-iff}
  $q\in B(p,r)\setminus\{p\}$ if and only if $0<d(q,p)<r$.

  \begin{lean}
    theorem mem_punctured_ball_iff (p q : X) (r : ℝ) :
        q ∈ ball p r \ {p} ↔ 0 < dist q p ∧ dist q p < r
  \end{lean}
\end{lemma}
...
\begin{definition} \label{an1:def:limit-interior-isolated-points}
  ...
  The set of limit points, or \textbf{derived set}, is denoted by $E'$ and represented by \texttt{derivedSet E}.
  A point is \textbf{interior} if some positive-radius ball about it lies in $E$; the interior $E^\circ$ is represented by \texttt{interior E}.
...
\begin{lemma} \label{an1:lem:interior-iff}
  $p\in E^{\circ}$ if and only if some $r>0$ satisfies $B(p,r)\subseteq E$.

  \begin{lean}
    theorem interior_iff (E : Set X) (p : X) :
        p ∈ interior E ↔ ∃ r > 0, ball p r ⊆ E
  \end{lean}
\end{lemma}

\begin{proof}
  A point is interior exactly when the set is its neighborhood; use the contained-ball characterization.
...
\begin{definition} \label{an1:def:open-closed-sets}
  ...
  We use the standard predicates \texttt{IsOpen E} and \texttt{IsClosed E} directly.
  The characterization of closed sets by $E^{\prime}\subseteq E$ is proved below.
\end{definition}

% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.isOpen_iff
\begin{lemma} \label{an1:lem:isOpen-iff}
  $E$ is open if and only if every $p\in E$ has a positive-radius ball contained in $E$.
...
\begin{proof}
  The metric open-set interface is the pointwise contained-ball characterization.
...
\begin{definition} \label{an1:def:closure}
  ...
  It is represented by \texttt{closure E}. We prove below that $\overline E=E\cup E^{\prime}$.
\end{definition}

\begin{definition} \label{an1:def:bounded-perfect-dense-sets}
  A set $E$ is \textbf{bounded} if there is a common upper bound for distances between its points; Lean uses \texttt{Bornology.IsBounded E}.
  Equivalently, when $X$ is nonempty, $E$ lies in a positive-radius ball, as proved below. Subsets of the empty space are bounded.
  A set is \textbf{perfect} if it is closed and each of its points is a limit point, represented by \texttt{Perfect E}. We prove below that this is equivalent to $E=E^{\prime}$.
  A set is \textbf{dense} if $\overline E=X$, represented by \texttt{Dense E}.
\end{definition}
...
\begin{definition} \label{an1:def:boundary}
  The \textbf{boundary} is $\partial E=\overline E\setminus E^{\circ}$, represented by \texttt{frontier E}.
...
\begin{definition} \label{an1:def:relative-open-closed-sets}
  ...
  In Lean, use \texttt{IsOpen} or \texttt{IsClosed} on the inverse image of $E$ under the inclusion \texttt{Subtype.val} from $Y$ to $X$, with $E\subseteq Y$ as a separate hypothesis.
\end{definition}
