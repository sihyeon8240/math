...
\begin{definition} \label{an1:def:limit-interior-isolated-points}
  ...
  \end{lean}
  \begin{lean}{$E^{\circ}$ is defined by contained positive-radius balls.}
...
\begin{proof}
  \begin{lean}{This is the defining condition for an interior point.}
    rfl
  \end{lean}
...
\begin{lemma} \label{an1:lem:interiorPoints-eq-interior}
  \begin{lean}{The ball-defined interior equals the standard interior.}
    theorem interiorPoints_eq_interior (E : Set X) : interiorPoints E = interior E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{A point lies in the standard interior when the set is a neighborhood of that point. The metric neighborhood interface says precisely that it contains a positive-radius ball.}
    ext p
...
\begin{lemma} \label{an1:lem:mem-openSets-iff}
  \begin{lean}{The locally defined open sets are exactly the sets satisfying the standard open-set predicate.}
    theorem mem_openSets_iff (E : Set X) : E ∈ openSets ↔ IsOpen E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{The metric open-set interface expresses the same condition: every point of the set has a ball contained in it.}
...
\begin{lemma} \label{an1:lem:mem-closedSets-iff}
  \begin{lean}{The locally defined closed sets are exactly the sets satisfying the standard closed-set predicate.}
    theorem mem_closedSets_iff (E : Set X) : E ∈ closedSets ↔ IsClosed E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{The preceding complement argument proves that closedness is equivalent to containing every limit point. Reverse that equivalence.}
...
\begin{lemma} \label{an1:lem:setClosure-eq-closure}
  \begin{lean}{The locally defined closure equals the standard closure.}
    theorem setClosure_eq_closure (E : Set X) : setClosure E = closure E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{The preceding ball argument identifies the standard closure with the union of the set and its limit points, exactly our definition.}
...
\begin{lemma} \label{an1:lem:subset-closure}
  \begin{lean}{Every set is contained in its closure.}
    theorem subset_closure (E : Set X) : E ⊆ closure E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{In the local definition, a point of the original set belongs to the left member of the union. Transfer this inclusion to the standard notation.}
...
\begin{definition} \label{an1:def:boundary}
  ...
  \begin{lean}{The boundary is the intersection of the two locally defined closures.}
...
\begin{proof}
  \begin{lean}{Replace each locally defined closure by its verified standard representation. The difference formula follows from the preceding complement-of-interior lemma.}
