\begin{definition} \label{an1:def:metric-space}
  ...
  The metric-space structure records the distance and its axioms.

  \begin{lean}
    abbrev MetricStructure (A : Type*) := MetricSpace A
  \end{lean}
  For every $n\in\N$, $\R^{n}$ has its Euclidean norm, including the unique vector when $n=0$.
...
\begin{proof}
  ...
  Write $x-z=(x-y)+(y-z)$ and apply the Euclidean norm triangle inequality. These norm facts are accepted finite-dimensional prerequisites.
...
\begin{definition} \label{an1:def:neighborhood-open-ball}
  ...
  $N_r(p)$ is the open ball centered at $p$.

  \begin{lean}
    abbrev neighborhood (p : X) (r : ℝ) : Set X := ball p r
  \end{lean}
  Removing the center gives $\widetilde N_r(p)$.
...
\begin{lemma} \label{an1:lem:deleted-neighborhood-iff}
  $q\in\widetilde N_r(p)$ if and only if $0<d(q,p)<r$.
...
\begin{definition} \label{an1:def:limit-interior-isolated-points}
  ...
  $p\in E^{\prime}$ means that every positive radius admits a distinct point of $E$ within that distance.

  \begin{lean}
    def limitPoints (E : Set X) : Set X :=
      {p | ∀ r : ℝ, 0 < r → ∃ q ∈ E, q ≠ p ∧ dist q p < r}
  \end{lean}
  $E^{\circ}$ is defined by contained positive-radius balls.

  \begin{lean}
    def interiorPoints (E : Set X) : Set X :=
      {p | ∃ r > 0, ball p r ⊆ E}
  \end{lean}
  $p\in E^{i}$ means $E\cap N_r(p)=\{p\}$ for some $r>0$. In particular $p\in E$.
...
\begin{lemma} \label{an1:lem:interior-iff}
  $p\in E^{\circ}$ if and only if some $r>0$ satisfies $N_r(p)\subseteq E$.
...
\begin{proof}
  This is the defining condition for an interior point.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.interiorPoints_eq_interior
\begin{lemma} \label{an1:lem:interiorPoints-eq-interior}
  The ball-defined interior equals the standard interior.

  \begin{lean}
    theorem interiorPoints_eq_interior (E : Set X) : interiorPoints E = interior E
  \end{lean}
\end{lemma}

\begin{proof}
  A point lies in the standard interior when the set is a neighborhood of that point. The metric neighborhood interface says precisely that it contains a positive-radius ball.
...
\begin{definition} \label{an1:def:open-closed-sets}
  ...
  These definitions express the stated conditions directly. Their agreement with the standard predicates is verified below.
  $\mathcal O(X)$ is the collection of open sets.

  \begin{lean}
    def openSets : Set (Set X) := {E | E ⊆ interiorPoints E}
  \end{lean}
  $\mathcal C(X)$ is the collection of closed sets.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.mem_openSets_iff
\begin{lemma} \label{an1:lem:mem-openSets-iff}
  The locally defined open sets are exactly the sets satisfying the standard open-set predicate.

  \begin{lean}
    theorem mem_openSets_iff (E : Set X) : E ∈ openSets ↔ IsOpen E
  \end{lean}
\end{lemma}

\begin{proof}
  The metric open-set interface expresses the same condition: every point of the set has a ball contained in it.
...
\begin{definition} \label{an1:def:closure}
  The \textbf{closure} of $E$ is $\overline E=E\cup E^{\prime}$. We define this union directly and prove its agreement with the standard closure below.
...
\begin{definition} \label{an1:def:bounded-perfect-dense-sets}
  ...
  When the ambient space has a point, a bounded set lies in a ball.

  \begin{lean}
    def bounded (E : Set X) : Prop :=
      Nonempty X → ∃ p : X, ∃ r > 0, E ⊆ ball p r
  \end{lean}
  A perfect set equals its set of limit points.

  \begin{lean}
    abbrev perfect (E : Set X) : Prop := E = limitPoints E
  \end{lean}
  A dense set has the whole ambient space as its closure.
...
\begin{proposition} \label{an1:thm:neighborhood-is-open}
  Every ball $N_r(p)$ is open. This also holds for nonpositive $r$, when the ball is empty.
...
\begin{proof}
  Suppose $q\in N_r(p)$. The remaining distance $\delta=r-d(q,p)$ is positive.
  ...
  If $d(s,q)<\delta$, the triangle inequality gives $d(s,p)\leq d(s,q)+d(q,p)<r$. Thus the ball of radius $\delta$ about $q$ stays inside $N_r(p)$.
...
\begin{proposition} \label{an1:thm:limit-point-neighborhood-contains-infinite-elements}
  If $p\in E^{\prime}$ and $r>0$, then $E\cap N_r(p)$ is infinite.
...
\begin{lemma} \label{an1:lem:closed-iff-limitPoints}
  The standard closed-set predicate is equivalent to $E^{\prime}\subseteq E$.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.mem_closedSets_iff
\begin{lemma} \label{an1:lem:mem-closedSets-iff}
  The locally defined closed sets are exactly the sets satisfying the standard closed-set predicate.

  \begin{lean}
    theorem mem_closedSets_iff (E : Set X) : E ∈ closedSets ↔ IsClosed E
  \end{lean}
\end{lemma}

\begin{proof}
  The preceding complement argument proves that closedness is equivalent to containing every limit point. Reverse that equivalence.
...
\begin{lemma} \label{an1:lem:closure-eq-union-limitPoints}
  $\overline E=E\cup E^{\prime}$, so the standard closure agrees with the textbook definition.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.MetricSpaces.setClosure_eq_closure
\begin{lemma} \label{an1:lem:setClosure-eq-closure}
  The locally defined closure equals the standard closure.

  \begin{lean}
    theorem setClosure_eq_closure (E : Set X) : setClosure E = closure E
  \end{lean}
\end{lemma}

\begin{proof}
  The preceding ball argument identifies the standard closure with the union of the set and its limit points, exactly our definition.
...
\begin{proof}
  In the local definition, a point of the original set belongs to the left member of the union. Transfer this inclusion to the standard notation.
...
\begin{proof}
  If $E\cap N_r(p)=\{p\}$, then $p\in E$ and no distinct point of $E$ is in that ball. This contradicts the limit-point condition, so $p\notin E^{\prime}$.
...
\begin{definition} \label{an1:def:boundary}
  ...
  The boundary is the intersection of the two locally defined closures.
...
\begin{proof}
  Replace each locally defined closure by its verified standard representation. The difference formula follows from the preceding complement-of-interior lemma.
...
\begin{lemma} \label{an1:lem:zero-dimensional}
  Any two points of $\R^{0}$ are equal, and every deleted neighborhood in $\R^{0}$ is empty.
...
\begin{lemma} \label{an1:lem:deleted-neighborhood-in-Rn-is-non-empty}
  For $n\in\N^{+}$, $p\in\R^{n}$, and $r>0$, the deleted neighborhood $\widetilde N_r(p)$ is nonempty.
...
\begin{lemma} \label{an1:lem:isolated-limit-compl}
  If every positive-radius deleted ball in $X$ is nonempty, then $E^{i}\subseteq(E^{c})^{\prime}$.
...
\begin{proof}
  For an isolated point $p$, choose $r>0$ with $E\cap N_r(p)=\{p\}$. For any $s>0$, take a distinct point within $\min(r,s)$ of $p$. It must lie outside $E$, and therefore witnesses that $p$ is a limit point of $E^{c}$.
...
\begin{proposition} \label{an1:thm:properties-of-boundary-in-rn}
  For $n\in\N^{+}$ and $E\subseteq\R^{n}$, $\partial E=E^{i}\cup(E^{c})^{i}\cup(E^{\prime}\cap(E^{c})^{\prime})$.
...
\begin{definition} \label{an1:def:relative-open-closed-sets}
  ...
  Relative openness means openness of $E$ as a subset of the subspace $Y$.

  \begin{lean}
    def relativeOpen (E Y : Set X) : Prop :=
      E ⊆ Y ∧ (Subtype.val : Y → X) ⁻¹' E ∈ openSets
  \end{lean}
  Relative closedness means closedness of that subset of $Y$.
...
\begin{proof}
  Suppose $E$ is open in $Y$. For each $p\in E$, choose $r_p>0$ with $Y\cap N_{r_p}(p)\subseteq E$.
