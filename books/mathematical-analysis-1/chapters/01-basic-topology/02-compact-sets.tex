\begin{definition} \label{an1:def:open-cover}
  ...
  An open cover consists of open sets whose union contains $E$.

  \begin{lean}
    abbrev openCover {ι : Type*} (E : Set X) (U : ι → Set X) : Prop :=
      (∀ i, IsOpen (U i)) ∧ E ⊆ ⋃ i, U i
  \end{lean}
  A subcover keeps only indices in $S$ and still contains $E$ in its union.
...
\begin{definition} \label{an1:def:compact}
  ...
  The definition below quantifies over index types in the same universe as the ambient space. The following bridge verifies its agreement with the standard predicate, which also handles covers with other index universes. The Lean context declares \texttt{universe u} and \texttt{variable \{X : Type u\} [MetricSpace X]}.
  $\mathcal K(X)$ consists of the compact subsets of $X$.
...
% Lean source: Textbooks.MathematicalAnalysis1.Chapter01.CompactSets.mem_compactSets_iff
\begin{lemma} \label{an1:lem:mem-compactSets-iff}
  The finite-open-subcover definition agrees with the standard compact-set predicate.

  \begin{lean}
    theorem mem_compactSets_iff (E : Set X) : E ∈ compactSets ↔ IsCompact E
  \end{lean}
\end{lemma}

\begin{proof}
  In either direction, unpack an open cover into openness and coverage and retain the same finite index set. The standard finite-subcover interface changes only the representation of compactness.
...
\begin{proof}
  ...
  If $x\in N_\delta(p)\cap E$, one of the selected balls contains $x$. Then $d(q,p)\leq d(q,x)+d(x,p)<r_q+\delta\leq 2r_q=d(q,p)$, a contradiction. Hence $N_\delta(p)\subseteq E^{c}$.
...
\begin{proof}
  If $X$ is nonempty, choose $p\in X$. The balls $N_{n+1}(p)$, for $n\in\N$, cover $E$: every real distance is below some natural number by the Archimedean property.
  ...
  Each $q\in E$ belongs to a selected ball. Its radius is at most the sum of all selected radii, so $d(q,p)<R$. Thus $E\subseteq N_R(p)$.
...
\begin{proof}
  Assume there is no such limit point. Then each $p\in E$ has a radius $r_p>0$ for which the only possible point of $F$ inside $N_{r_p}(p)$ is $p$ itself. Otherwise every radius would have a distinct witness, making $p$ a limit point.
...
\begin{proof}
  ...
  If $d(x^{\prime},x)<\delta$ and $y^{\prime}\in L$, choose a selected ball containing $y^{\prime}$. Both coordinate distances are below its radius, so $(x^{\prime},y^{\prime})$ lies in its original cover member. Thus finitely many members cover the whole strip $N_\delta(x)\times L$.
...
\begin{proof}
  A compact set is closed and bounded by the metric-space results above. Conversely, suppose $E$ is closed and bounded. Euclidean space has a zero vector even in dimension zero, so choose a ball $N_r(p)$ containing $E$.
  ...
  The box with coordinate intervals $[p_j-r,p_j+r]$ is compact. Each coordinate difference is bounded in absolute value by the Euclidean norm, so a point in $N_r(p)$ lies in this box. Hence $E$ is a closed subset of a compact set and is compact.
