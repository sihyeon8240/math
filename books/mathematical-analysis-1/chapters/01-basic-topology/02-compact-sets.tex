...
\begin{lemma} \label{an1:lem:mem-compactSets-iff}
  \begin{lean}{The finite-open-subcover definition agrees with the standard compact-set predicate.}
    theorem mem_compactSets_iff (E : Set X) : E ∈ compactSets ↔ IsCompact E
  \end{lean}
\end{lemma}

\begin{proof}
  \begin{lean}{In either direction, unpack an open cover into openness and coverage and retain the same finite index set. The standard finite-subcover interface changes only the representation of compactness.}
    constructor
    · intro h
...
\begin{proof}
  ...
  \begin{lean}{For $n+1$ coordinates, separate coordinate $0$ from the remaining $n$. The map adjoining that first coordinate is continuous: each output-coordinate distance is bounded by the maximum product distance of the inputs, so $\delta=\varepsilon$ suffices. The interval for coordinate $0$ and the remaining box are compact, so their product is compact.}
    | succ n ih =>
...
\begin{lemma} \label{an1:lem:continuous-to-euclidean}
  \begin{lean}{The coordinate identity from the finite-product metric on $\mathbb R^n$ to the Euclidean metric is continuous, including $n=0$.}
...
\begin{proof}
  \begin{lean}{Fix $\varepsilon>0$ and take $\delta=\varepsilon/(n+1)>0$. For two vectors within product distance $\delta$, let $D$ be that product distance and $d$ their Euclidean distance.}
    apply Metric.continuous_iff.mpr
    intro x ε hε
  ...
  \begin{lean}{Each coordinate distance is at most $D$. Squaring these nonnegative inequalities and summing the $n$ terms gives $d^2\le nD^2$ by the Euclidean distance formula.}
  ...
  \begin{lean}{Since $n\le(n+1)^2$ and both distances are nonnegative, $d\le(n+1)D$. The choice $D<\varepsilon/(n+1)$ therefore gives $d<\varepsilon$. This estimate also covers the empty coordinate type.}
...
\begin{proof}
  \begin{lean}{The preceding distance estimate proves continuity of the coordinate identity from the product description to the Euclidean description. Its image is exactly the same coordinate box, so the preceding result and compactness of continuous images apply. This includes $n=0$.}
