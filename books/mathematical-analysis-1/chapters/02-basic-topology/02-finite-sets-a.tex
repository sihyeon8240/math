\begin{theorem} \label{an1:thm:bijection-equivalence-relation}
  Define $\sim$ by
  \[
    ...
    \item \textbf{Symmetry}: Suppose $X \sim Y$. Then
          \[
            \begin{array}{c}
              ...
              \by{\Cref{an1:thm:inverse-function-is-bijective}} \Downarrow \\
...
\begin{remark}
  ...
  \cref{an1:thm:bijection-equivalence-relation} is a \textbf{proper-class} relation rather than a set.
  Its restriction to any set $S$ becomes an equivalence relation on $S$.
\end{remark}

\begin{definition} \label{an1:def:finite-sets}
  Recall the relation $\sim$ defined in \cref{an1:thm:bijection-equivalence-relation}.
...
\begin{definition} \label{an1:def:infinite-countable-uncountable-sets}
  A set $X$ is said to be
  \begin{itemize}
    \item \textbf{infinite} if $\neg\paren*{\abs*{X} < \aleph_{0}}$
    \item \textbf{countable} if $\N \sim X$
    \item \textbf{at most countable} if $\abs*{X} < \aleph_{0} \lor \N \sim X$
    \item \textbf{uncountable} if $\neg\paren*{\abs*{X} < \aleph_{0} \lor \N \sim X}$
  \end{itemize}
  The notation
  \begin{itemize}
...
\begin{lemma} \label{an1:lem:subset-of-Nn-is-finite}
  \[
...
\begin{proof}
  Define
  \[
    \forall n \in [0, \infty)_{\N}, P(n) \iff \forall A \subseteq [1, n]_{\N}, \abs*{A} < \aleph_{0}
  \]
  Then
  \begin{itemize}
    \item \textbf{Base case:} Define $f: [1, 0]_{\N} \to [1, 0]_{\N}$ by
          \[
            \forall k \in [1, 0]_{\N}, f(k) \coloneqq k
          \]
          Then
          \[
            \begin{array}{c}
              [1, 0]_{\N} = \vnot                           \\
              \Downarrow                                    \\
              f: [1, 0]_{\N} \xrightarrow{\sim} [1, 0]_{\N} \\
              \Downarrow                                    \\
              \abs*{[1, 0]_{\N}} < \aleph_{0}
            \end{array}
          \]
          Therefore,
          \[
            \begin{array}{c}
              ...
              \Downarrow                                             \\
              \forall A \subseteq [1, 0]_{\N}, \abs*{A} < \aleph_{0} \\
              \Downarrow                                             \\
              P(0)
            \end{array}
          \]
    \item \textbf{Inductive step:} Suppose
          ...
          \begin{itemize}
            ...
            \item Otherwise, define
                  ...
                  \[
                    \forall k \in [1, m + 1]_{\N}, g(k) \coloneqq \begin{dcases}
                      n + 1, & \text{if } k = m + 1 \\
                      f(k),  & \text{otherwise}
                    \end{dcases}
                  \]
...
\begin{theorem} \label{an1:thm:subset-of-finite-set-is-finite}
  \[
...
\begin{corollary} \label{an1:cor:infinite-subset-implies-infinite}
  \[
