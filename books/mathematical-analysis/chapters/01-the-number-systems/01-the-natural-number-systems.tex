...
\begin{theorem} \label{an:thm:mathematical-induction}
  Suppose $P(n)$ is a statement about $n \in \N$. Define
  \[
...
\begin{definition} \label{an:def:operations-on-N}
  Suppose $n, m \in \N$.
  \begin{itemize}
    \item The \textbf{addition} $+$ on $\N$ is defined by
          \begin{itemize}
            \item $n + 1 \coloneqq s(n)$
            \item $n + s(m) \coloneqq s(n + m)$
          \end{itemize}
    \item The \textbf{multiplication} $\cdot$ on $\N$ is defined by
          \begin{itemize}
...
\begin{theorem} \label{an:thm:well-ordering-property}
  ...
  \]
  \leanverified{an:thm:well-ordering-property}
\end{theorem}
