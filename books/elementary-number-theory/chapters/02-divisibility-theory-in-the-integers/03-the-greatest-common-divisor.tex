...
% Lean source: Textbooks.ElementaryNumberTheory.Chapter02.GreatestCommonDivisor.gcd_eq_int_gcd
...
\begin{proof}
  If the inputs are not both zero, the locally constructed gcd divides each input, so it divides the standard gcd. The standard gcd also divides each input, so our proved common-divisor property gives the reverse divisibility. Two natural numbers dividing one another are equal.
