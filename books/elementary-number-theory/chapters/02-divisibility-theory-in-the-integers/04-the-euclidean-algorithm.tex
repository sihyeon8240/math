We retain the nonnegative gcd, with $\gcd(0,0)=0$.
Pairs with the same common divisors have equal gcds: for nonzero pairs,
each gcd is at most the other; a zero pair is characterized by having zero
as a common divisor.
...
% Lean source: Textbooks.ElementaryNumberTheory.Chapter02.LeastCommonMultiple.lcm_eq_int_lcm
\begin{lemma} \label{nt:lem:lcm-eq-int-lcm}
  For all integers $a,b$, the lcm constructed above agrees with Mathlib\textquotesingle s standard integer lcm, including zero inputs.

  \begin{lean}
    theorem lcm_eq_int_lcm (a b : ℤ) : lcm a b = Int.lcm a b
  \end{lean}
\end{lemma}

\begin{proof}
  If either input is zero, both definitions give zero.
  ...
  Otherwise the standard lcm is a positive common multiple. It divides every positive common multiple $c$, so it is at most $c$. The previously proved uniqueness of the least positive common multiple identifies it with our construction.
